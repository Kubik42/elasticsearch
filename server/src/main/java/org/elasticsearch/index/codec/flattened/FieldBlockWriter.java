...
/**
 * Writes one column (a sub-field of flattened) directly into an existing {@link IndexOutput}.
 *
 * <p>Blocks are written straight into the caller-supplied output — no temp files, no splice. The caller retains ownership of the output.
 *
 * <p>One instance is reused for every column of a field: call {@link #reset(IndexOutput)} at each column boundary rather than allocating a
 * new writer. The block-accumulation arrays are pre-sized from the configured thresholds ({@code 2 * 4 * maxDocsPerBlock +
 * targetBlockBytes} bytes) and the ZSTD compressor is expensive to allocate, so allocating per column produces hundreds of megabytes of
 * short-lived garbage per flush on fields with large sub-field cardinality.
 *
...
final class FieldBlockWriter {
    ...
    /**
     * Reusable scratch buffer for the metadata region. Allocated once in the constructor and reused across blocks/columns; never cleared
     * by {@link #reset}.
     */
    ...
    /**
     * Constructs a writer that emits blocks directly into {@code out}. No temp files are created; the caller retains ownership of
     * {@code out} and must keep it open until after {@link #finish()} returns.
     *
     * <p>Prefer allocating one instance and calling {@link #reset(IndexOutput)} at each column boundary to avoid per-column array and
     ...
     *
     * <p>Must only be called immediately after {@link #finish()} has been called (or before any slots have been added for the very first
     * column). Reuses all pre-allocated arrays and the ZSTD compressor from the previous column.
     */
     ...
     *
     * <p>{@code slotLens[slotLensOff .. slotLensOff+slotCount)} gives the byte length of each slot, with {@code -1} meaning a null slot.
     * {@code values[valuesOff .. valuesOff+valuesLen)} contains the concatenated raw value bytes for the non-null slots (null slots
     * contribute nothing).
     *
     * <p>The block-flush trigger fires before appending, at the doc boundary, so no document's slots are ever split across two blocks.
     *
    ...
    /**
     * Finishes the column: flushes the tail block and appends the block index (8 bytes per entry). Must be called exactly once, after all
    ...
    /**
     * Returns the {@link ColumnAddress} describing this column's on-disk position. Must be called after {@link #finish()}.
     */
    ...
    private void flushCurrentBlock() throws IOException {
        ...
        for (int i = 0; i < numDocsInBlock; i++) {
            ...
            if (i > 0 && blockDocIds[i] != blockDocIds[i - 1] + 1) contiguous = false;
        }
        ...
        // Encode slot counts and value lengths into the metadata buffer before writing flags, because FLAG_META_COMPRESSED depends on the
        // encoded size.
        ...
        if (allSingleSlot == false) {
            ...
            bitsPerSlot = Math.max(1, Integer.SIZE - Integer.numberOfLeadingZeros(maxSlot));
        }
        ...
        final int bitsPerLen = Math.max(1, Integer.SIZE - Integer.numberOfLeadingZeros(maxLen));
        ...
        currentOut.writeVInt(numDocsInBlock);
